...
class AccountService {
  Future<GenericResponse> login(Account account) async {
    try {
      // read json as string
      String jsonString =
          await rootBundle.loadString('lib/assets/jsons/accounts.json');
      // string to json
      ...
      for (Account a in accounts) {
        if (account.username == a.username && account.passwordHash == a.passwordHash) {
          return GenericResponse(
              success: true, data: a, message: 'Login exitoso', error: null);
        }
      }
      // account not found
      return GenericResponse(
